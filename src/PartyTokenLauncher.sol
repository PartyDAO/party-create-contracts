...
contract PartyTokenLauncher is Ownable, IERC721Receiver {
    ...
    function _finalize(uint32 launchId, Launch memory launch) private {
        ...
        calls[1] = abi.encodePacked(POSTION_MANAGER.refundETH.selector);
        bytes memory mintReturnData = IMulticall(address(POSTION_MANAGER)).multicall{ value: amountForPool }(calls)[0];
